...
plot_model(simpleLG,type="pred",terms=c("Education","Family","Mortgage"))

plot_model(simpleLG,type="pred",terms=c("Education","Education","Income"))
...
#INTERACTION with MORTGAGE
ComplexInteractionl2<-glm(Personal.Loan~Securities.Account+CD.Account+CreditCard+Education+Income+CCAvg+Education*Mortgage,data=train,family="binomial")   #complex
#summary(ComplexInteractionl)
all_results2$Interaction=results(ComplexInteractionl,FALSE)
roc_interaction=ROC_predict(ComplexInteractionl)
...
ldaqda_predict <- function(model){
  pred_model<-predict(model,newdata=test)$class
  accuracy=round((mean(pred_model==test$Personal.Loan)),3)
  Sensitivit=round((sensitivity(pred_model, test$Personal.Loan)),3)
  specificit= round((specificity(pred_model, test$Personal.Loan)),3)
...
qda_1<- qda( Personal.Loan ~ Income + CCAvg  + Mortgage, data = train)
...
legend("bottomright",legend=c("LDA_NumericOnly", "LDA_all","QDA_Numeric","QLD_All variables"),col=c("black","red","orange","blue"),lty=1,lwd=1)
abline(a=0, b= 1)
```


+ ***Log and interaction and log of Mortage and income seemed to be the best ones from the interaction, logs and LDA ***
...
legend("bottomright",legend=c("Interaction", "LogIncomeAndMortgage","log and Interaction","LDAnum", "QDAall","StepWise"),col=c("black","red","orange","blue","yellow","pink"),lty=1,lwd=1)
abline(a=0, b= 1)
```



+ ***Polyboth3 was the best model***
...
+Setting both CCAvg and Income up to 3 polynomials was the best model followed by setting income to the 2nd polynomial. The model where both CCAvg and Income were set to 3 poly, does coincide with the best Specificity and Accuracy on the test data. Specificity of 86% compared to a specificity of 80% in the best model (StepWise) in part one. We care the most about specificity because the reference the algorithm chosen was "NO" and the in the purpose of this project we care about being able to detect those that will take the loan. the highe sensitivity and lower specificity is due to having  90% of the data with No personal loans while only 10% had yes in the data. 

```{r, echo=FALSE}

#plot(roc_PolyIncome2, col = "green", xlim = c(0, 0.3), ylim = c(0.7, 1.0))
plot(roc_stepWiseAIC, col = "chartreuse", xlim = c(0, 0.3), ylim = c(0.7, 1.0))
plot(roc_logIncomeMortgage, col = "blueviolet", add = TRUE, xlim = c(0, 0.3), ylim = c(0.7, 1.0))
plot(roc_logAndInteraction, col = "aquamarine", add = TRUE, xlim = c(0, 0.3), ylim = c(0.7, 1.0))
plot(roc_PolyIncome3, col = "blue", add = TRUE, xlim = c(0, 0.3), ylim = c(0.7, 1.0))
plot(roc_PolyBoth3, col = "coral", add = TRUE, xlim = c(0, 0.3), ylim = c(0.7, 1.0))
plot(roc_extreme, col = "greenyellow", add = TRUE, xlim = c(0, 0.3), ylim = c(0.7, 1.0))
legend("bottomright", legend = c( "StepWise", "LogIncome&Mortgage", "Log&interaction", "polyIncome3", "PolyBoth3", "complexmoel"), 
       col = c( "chartreuse", "blueviolet", "aquamarine", "blue", "coral","greenyellow"), 
       lty=1, lwd=1)
abline(a=0, b= 1)
```
...
all_combined=all_results %>% cbind(all_results2,all_results3)

all_combined[c(1,3,10,11,16,20)]
```
...
#randomFrst <- randomForest(Personal.Loan ~ ., data = train,  importance = TRUE, proximity = TRUE)
#fit.pred<-predict(randomFrst,newdata=test,type="response")
#class.model<-factor(ifelse(fit.pred>0.3,"Yes","No"),levels=c("No","Yes"))
#confusionMatrix(fit.pred,test$Personal.Loan, positive="Yes")
