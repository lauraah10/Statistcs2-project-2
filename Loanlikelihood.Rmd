...
###########Testing if it is using yes as the refference####
#fit.pred<-predict(ComplexInteractionl,newdata=test,type="response")
#class.model<-factor(ifelse(fit.pred>0.3,"Yes","No"),levels=c("No","Yes"))
#confusionMatrix(class.model,test$Personal.Loan)
#confusionMatrix(class.model,test$Personal.Loan, positive="Yes")
...
ComplexLog<-glm(Personal.Loan~Education+log(Income)+CCAvg+ Family+ MortgageLogged+Securities.Account+CD.Account+CreditCard,data=PersonalLoan2,family="binomial")   #complex
...
ComplexLogInteraction<-glm(Personal.Loan~Education+log(Income)+CCAvg+ Family*MortgageLogged+Securities.Account+CD.Account+CreditCard,data=PersonalLoan2,family="binomial")   #complex
...
legend("bottomright",legend=c("Interaction", "LogIncomeAndMortgage","log and Interaction","LDA", "QDA"),col=c("black","red","orange","blue","Yellow"),lty=1,lwd=1)
abline(a=0, b= 1)
```
...
```
```{r}
all_results2 %>% select(Criterion, Interaction, LogIncomeMortgage, LogIncomeMortgage, LogAndInteraction, LDA, QDA2)
