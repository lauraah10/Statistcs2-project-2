...
```{r, echo=FALSE}
# Extra plots
ggplot(PersonalL,aes(x=Income,y=Personal.Loan,colour=Family))+geom_point()+
  geom_smooth(method="loess",size=1,span=1.5)+
  ylim(-.2,1.2)+
  facet_wrap(~Family)

ggplot(PersonalL,aes(x=Income,y=Personal.Loan,colour=Education))+geom_point()+
  geom_smooth(method="loess",size=1,span=1.5)+
  ylim(-.2,1.2)+
  facet_wrap(~Family)
...
+ ***Polyboth3 was the best model for specificity is the best***
...
+ ***The best model from the first part vs the best of the second set***
